...
public final class BuyShopBankBalancePurchaseService {
    ...
    public ShopTransactionResult sellToShop(ServerPlayer player, UUID commercialBlockId, int offerSlot, String idempotencyKey, LocalDate businessDate) throws SQLException {
        ...
        try (Connection connection = EconomyDatabase.getConnection()) {
            ...
            try {
                ...
                FinancialOperationResult payment = accountFinancialService.withdraw(player.getUUID(), links.linkedAccountId(), amount, idempotencyKey);
                if (payment.type() != FinancialOperationResultType.COMPLETED && payment.type() != FinancialOperationResultType.DUPLICATE_COMPLETED) {
                    playerItems.insert(player, item, offer.quantityPerOperation());
                    return paymentFailure(payment);
                }
    ...
    private ShopTransactionResult paymentFailure(FinancialOperationResult payment) {
        if (payment.type() == FinancialOperationResultType.INSUFFICIENT_BALANCE) {
